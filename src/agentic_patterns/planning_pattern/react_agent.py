...
class ReactAgent:
    ...
    def run(
        ...
        """
        # 1. Build the initial user prompt
        user_prompt = build_prompt_structure(
            prompt=user_msg, role="user", tag="question"
        )
        # 2. Build system prompt with tool signatures
        if self.tools:
            self.system_prompt += (
        ...
        # 3. Run the ReAct loop for max_rounds
        ...
        if self.tools:
            for _ in range(max_rounds):
                ...
                #  Extract the  model output: check if it is:
                #  (1) thought (2) tool calls or (3) response
